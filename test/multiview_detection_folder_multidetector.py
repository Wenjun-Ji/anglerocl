...
"""
Multi-Detector Multiview Detection Script (Using PerspectiveViewGenerator) with AASR Analysis
...
2. For each texture image in the folder:
   ...
   b. Detects stop signs using selected detector (YOLOv3/YOLOv5/YOLOv10/Faster R-CNN/DETR)
   c. Saves detection confidence scores to separate CSV files
   d. Calculates AASR metric (Angular Attack Success Rate)
   e. Creates confidence-angle curve plots
   f. Optionally saves all rendered images
3. Aggregates results from all textures
"""
...
class MMDetectionDetector:
    ...
    def detect_stop_sign(self, image):
        ...
        """
        # Direct call without additional preprocessing
...
def parse_args():
    parser = argparse.ArgumentParser(description='Multi-detector Stop Sign Detection with AASR')
    parser.add_argument('--texture-folder',
                      type=str,
                      default="/zhaotingfeng/jiwenjun/anglerocl/data/stable_diffusion_1.5/stop_sign_prompts/blue_square_stop_sign",
...
def save_rendered_image(rgb_image, save_path, title=""):
...
def load_detector(args, device):
    """Load the appropriate detector based on arguments"""
...
def batch_detect_stop_sign_unified(model, images, detector_type, batch_size=128, target_class_id=11):
    ...
    if detector_type in ['yolov3', 'faster_rcnn', 'detr']:
        ...
        # If batch_size is smaller than number of images, process in batches
...
def plot_confidence_curve(angles, confidences, plot_path, title=None):
    ...
    plt_title = 'Stop Sign Detection Confidence vs. Viewing Angle'
    if title:
        plt_title += f" - {title}"
    plt.title(plt_title)
    plt.grid(True)
...
def calculate_aasr_for_texture(angles, confidences, thresholds, output_dir=None):
    ...
    try:
        ...
        # If output directory is provided, save results as CSV
        if output_dir:
            os.makedirs(output_dir, exist_ok=True)

            # 1. Save summarized AASR results to CSV
            ...
            with open(metrics_path, 'w', newline='', encoding='utf-8') as f:
                ...
                # Write detailed AASR information for each threshold
...
def process_texture_file(texture_path, model, output_dir, args, device):
    ...
    try:
        ...
        # Render front view
        front_view_angle = 0
        front_views, _, _ = renderer(
            texture_tensor,
            longitudes=[front_view_angle],
            latitudes=[0],
            distance=args.renderer_distance
        )
        front_view_image = front_views[0, 0]

        # Detect front view
        front_view_confidence = detect_stop_sign_unified(model, front_view_image, args.detector, args.target_class_id)
        ...
        save_rendered_image(
            front_view_image,
            front_view_path,
            title=f"Front View (Azimuth: {front_view_angle}°), Confidence: {front_view_confidence:.4f}"
        )
        ...
        with open(csv_path, 'w', newline='') as csvfile:
            ...
            # Render all angles at once
            print(f"  Rendering all {len(azim_angles)} angles...")
            ...
            view_images = []
            for i in range(len(azim_angles)):
                view_images.append(rendered_views[i, 0])

            # Use batch detection
            all_confidences = batch_detect_stop_sign_unified(
                model, view_images, args.detector, batch_size=args.batch_size, target_class_id=args.target_class_id
            )
...
def main():
    ...
    # Automatically append detector name to output_dir
    args.output_dir = os.path.join(args.output_dir, args.detector)
    ...
    # Prepare model pool (for multi-threading)
    model_pool = []
    if args.num_workers > 1 and args.detector in ['yolov3', 'faster_rcnn', 'detr']:
        # Create a model instance for each worker (max 4)
    ...
    if args.num_workers <= 1:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=args.num_workers) as executor:
            ...
            for i, texture_file in enumerate(texture_files):
                ...
                worker_model = model_pool[i % len(model_pool)]
    ...
    if args.enable_aasr and AASR_AVAILABLE:
        try:
            ...
            # Collect AASR results from all textures
    ...
    if len(all_results) > 1:
        ...
        if example_result:
            ...
            # Save angle-confidence data for all textures to CSV
                ...
                for angle in angles:
                    ...
                    for result in all_results:
                        if 'angle_confidences' in result and angle in result['angle_confidences']:
                            ...
                            row.append('')
                    csv_writer.writerow(row)
            print(f"Angle-confidence data for all textures saved to {all_textures_data_path}")
            ...
            plt.xlabel('Azimuth Angle (degrees)')
            plt.ylabel('Detection Confidence')
            plt.title(f'Stop Sign Detection Confidence vs. Viewing Angle - All Textures ({args.detector.upper()})')
            plt.grid(True)
            plt.xlim(-90, 90)
            plt.ylim(0, 1.05)
            ...
            plt.title(f'Mean Stop Sign Detection Confidence vs. Viewing Angle ({args.detector.upper()})', fontsize=14)
            plt.grid(True)
            plt.xlim(-90, 90)
            plt.ylim(0, 1.05)
            ...
            # 3. Create heatmap showing confidence for all textures at different angles
            ...
            for result in all_results:
                if 'angle_confidences' in result:
                    ...
                    # Get confidences for this texture at all angles
                    confidences = [result['angle_confidences'][angle] for angle in angles]
            ...
            plt.xlabel('Azimuth Angle (degrees)')
            plt.ylabel('Texture')
            plt.title(f'Detection Confidence Heatmap ({args.detector.upper()})')
            ...
            plt.title(f'Maximum and Average Detection Confidence by Texture ({args.detector.upper()})')
            plt.xticks(x, sorted_texture_names, rotation=45, ha='right', fontsize=8)
            plt.ylim(0, 1.05)
            plt.grid(True, axis='y', linestyle='--', alpha=0.7)
            plt.legend()

            # Add value labels to maximum bars
            for bar in bars1:
